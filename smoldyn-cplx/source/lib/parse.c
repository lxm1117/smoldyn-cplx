...
int Parse_ReadLine(ParseFilePtr *pfpptr,char *word,char **line2ptr,char *erstr) {
	int itct,ans,er,skip,toolong,windows;
	char *linetest,*line,*line2,ch,*chptr,*chptr2;
	...
	ans=er=0;
	line=pfp->line;
	linetest=fgets(line,STRCHAR,pfp->fptr);
	skip=0;
	if(linetest) {																// pre-process line
		pfp->lctr++;
		strcpy(pfp->linecopy,line);
		toolong=(strchr(line,'\n')==NULL);
		if(strchr(line,'\r')) {											// check for Mac or Windows format
			chptr=strchr(line,'\r');
			if((chptr2=strchr(chptr,'\n')) && chptr2==chptr+1) windows=1;
			else windows=0;
			strcpy(chptr,"\n");
			strcpy(pfp->linecopy,line);
			if(windows) {
				CHECKS(0,"%s is Windows format file. Chage to Unix format with: perl -pi -e 's/\\r//g' %s",pfp->fname,pfp->fname); }
			CHECKS(0,"%s is Macintosh format file. Changed to Unix format with: perl -pi -e 's/\\r/\\n/g' %s",pfp->fname,pfp->fname); }
		...
		if(!skip) {
			CHECKS(!toolong,"Line exceeds maximum allowable length of %i characters; make sure this line end with a '\n'.",STRCHAR);
