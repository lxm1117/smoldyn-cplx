...
int checksurfaces(simptr sim,int ll, int reborn){
	...
	int s,m,nmol,result,it,m_next,s0,d;	
	...
	while(m<nmol) {
		...
		//if((mptr->pos!=mptr->pos_tmp || mptr->from!=NULL) && mptr->complex_id==-1){
		...
		mptr_tmp=mptr;
		s0=0;
		// since for a complex dif_molec and dif_bind always exist, the following code block doesn't make sense
		/*
		...
		*/
		...
		//if(mptr->tot_sunit==1 && mptr->to!=NULL) {
		//	for(d=0;d<sim->dim;d++) mptr->to->pos[d]=mptr->pos[d]; }
  		if(result!=0){
