...
complexptr complexalloc(simptr sim, moleculeptr mptr){
	...
	cplxptr->dif_molec=mptr;  //NULl
	cplxptr->dif_bind=mptr;
...
int moladdsites(simptr sim, char *species, char *site_name, int sitecode){
	...
	if(sitecode+1>mols->spsites_num[sp_indx]){
		...
		for(;i<sitecode+1;i++){
			...
			else if(!strcmp(site_name,"ES")) {
				newsp_sites_binding[i]=2;
				strcpy(newsp_sites_name[i],site_name);
			}
			else{	
...
int molsupdate(simptr sim) {
	...
	if(mols) {
		if(mols->condition<=SClists) {
			...
			if(er>0){
				printf("err=%d in molsupatelists\n", er);
				return er;
			}
		...
		if(mols->condition==SCparams) {
			...
			if(er){
				printf("err=%d in molsupdateparams\n", er);
				return er;
			}
...
moleculeptr getnextmol_cplx(molssptr mols, int sunit, int ident) {
	...
	for(s=0;s<sunit;s++){	
		...
		for(k=0;k<spsites_num;k++){			// default unbound, sites_state=1;
			mptr_tmp->sites[k]->site_type=mols->spsites_binding[ident][k];
...
int addmol_cplx(simptr sim,int num_mol,int sunit,int bind_num,int *ident,int *sites,double *poslo,double *poshi,int sort, int ident_free, int sites_free) {
	...
	for(m=0;m<num_mol;m++) {
		...
		if(bind_num>1) {
			...
			if(sim->events){
				fprintf(sim->events,"rxn_time=%f start sindex=%d bound_ident=%s bound_serno=%d bound_state=%d  pos[0]=%f pos[1]=%f pos[2]=%f complex_id=%d\n", sim->time, mptr_bound->s_index, sim->mols->spname[mptr_bound->ident], mptr_bound->serno, mptr_bound->sites_val, mptr_bound->pos[0], mptr_bound->pos[1], mptr_bound->pos[2], mptr_bound->complex_id);}
		}		
		mptr->sites_val=molecsites_state(sim->mols,mptr);
		if(sim->events)	{
			fprintf(sim->events,"rxn_time=%f start sindex=%d ident=%s serno=%d sites_val=%d pos[0]=%f pos[1]=%f pos[2]=%f complex_id=%d\n", sim->time,mptr->s_index, sim->mols->spname[mptr->ident],mptr->serno,mptr->sites_val,mptr->pos[0],mptr->pos[1],mptr->pos[2], mptr->complex_id);}
...
int updatecompartmol_cplx(simptr sim,int serno, int bind_num,int *ident,int *sites_val,compartptr cmpt){
	...
	if(bind_num>1){
		...
		mptr_bound->sites_val=molecsites_state(sim->mols,mptr_bound);	
		if(sim->events){
			fprintf(sim->events,"rxn_time=%f start index=%d bound_ident=%s bound_serno=%d sites_val=%d pos[0]=%f pos[1]=%f pos[2]=%f complex_id=%d\n", sim->time,mptr_bound->s_index,sim->mols->spname[mptr_bound->ident],mptr_bound->serno,mptr_bound->sites_val,mptr_bound->pos[0],mptr_bound->pos[1],mptr_bound->pos[2], mptr_bound->complex_id);
		}
	}
	mptr->sites_val=molecsites_state(sim->mols,mptr);
	if(sim->events){
		fprintf(sim->events,"rxn_time=%f start index=%d ident=%s serno=%d sites_val=%d pos[0]=%f pos[1]=%f pos[2]=%f complex_id=%d\n", sim->time,mptr->s_index,sim->mols->spname[mptr->ident],mptr->serno,mptr->sites_val,mptr->pos[0],mptr->pos[1],mptr->pos[2],mptr->complex_id);		
	}
	...
	for(ll=0;ll<mols->nlist;ll++)
		if(mols->diffuselist[ll]){
			...
			for(m=0;m<nmol;m+=mptr->tot_sunit){
				...
				else if(mptr->bind_id>=0 && mptr->bind_id!=mptr->ident) 
					i=mptr->bind_id;
				else
					i=mptr->ident;
				ms=mptr->mstate;
				...
				*/
				if(!difm[i][ms]){															// isotropic diffusion
					...
					if(difc_type==1) {
						if(mptr->tot_sunit==1 && mptr->pos==mptr->pos_tmp && mptr->from==NULL)  {
							for(d=0;d<dim;d++) {
									mptr->prev_pos[d]=mptr->pos[d];	
									mptr->pos[d]+=sqrt(2.0*difc*sim->dt)*gtable[randULI()&ngtablem1];	
									/*
									if(mptr->to!=NULL){
										mptr->to->prev_pos[0]=mptr->to->pos[d];
...
void complex_pos_init(simptr sim, moleculeptr mptr,compartptr cmpt){
	...
	if(sim->mols->complex_radius>0){
		r=sim->mols->complex_radius;
		printf("complex_radius=%f\n",r);		
	}
	else r=8.0;
