...
int simreadstring(simptr sim,ParseFilePtr pfp,const char *word,char *line2) {
	...
	else if(!strcmp(word,"complex_radius")){
		int cr;
		CHECKS(sim->mols,"need to enter species before complex_radius");
		itct=sscanf(line2,"%d",&cr);
		sim->mols->complex_radius=cr;
	}
